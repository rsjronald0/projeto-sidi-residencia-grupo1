...
# Verifica se a database sidi_service já existe, se não existe então é criada
for m in mycursor:
    for v in m:
        if (v == 'sidi_service'):
            db_exists = True
...
if (db_exists == False):
  mycursor.execute("CREATE DATABASE sidi_service")

# Conecta à database sidi_service após garantir sua existência
mydb = mysql.connector.connect(
  host="localhost",
  user="sidi",
  password="sidi",
  database="sidi_service"
)
